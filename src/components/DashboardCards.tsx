...
const DashboardCards: React.FC<DashboardCardsProps> = ({ cardData, onCardExpand, isDirector = false }) => {
  const getCards = () => {
    const baseCards = [
      { id: 'pre-review', title: 'Pre-Review', icon: FileText, count: cardData.preReviewCount, color: 'bg-blue-500' },
      { id: 'double-blind', title: 'Double-Blind', icon: Users, count: cardData.doubleBlindCount, color: 'bg-green-500' },
      ...
      { id: 'upload', title: 'Upload', icon: Upload, count: cardData.uploadCount, color: 'bg-purple-500' },
    ...
    if (!isDirector) {
      return [
        ...baseCards.slice(0, 2),
        { id: 'accepted', title: 'Accepted', icon: CheckCircle, count: cardData.acceptedCount || 0, color: 'bg-teal-500' },
        ...baseCards.slice(2)
      ];
    }
