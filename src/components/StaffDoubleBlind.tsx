...
const StaffDoubleBlind: React.FC = () => {
  ...
  const handleProceedToLayout = () => {
    if (selectedManuscript && dateAccepted && layoutArtist) {
      const acceptedManuscript = {
        ...selectedManuscript,
        status: 'accepted',
        layoutDetails: {
          dateAccepted,
  ...
  const handleReject = () => {
    if (selectedManuscript) {
      updateManuscriptStatus(selectedManuscript.id, 'rejected');
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {selectedManuscript.revisionStatus ? (
                  <span className="inline-block mt-1 px-2 py-1 rounded-full text-sm bg-blue-100 text-blue-800">
                    {selectedManuscript.revisionStatus}
                  </span>
                ) : (
                  <p className="text-gray-500 italic">No revision status</p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg w-[500px]">
            ...
            <div className="mt-6 flex justify-end space-x-4">
              ...
              <button
                ...
                className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition-colors"
                disabled={!dateAccepted || !layoutArtist}
              >
                Proceed To Layout
              </button>
            </div>
      ...
      {isRejectModalOpen && selectedManuscript && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-[500px]">
            ...
            <div className="mb-4">
              <h4 className="text-lg font-semibold mb-2">Are you sure you want to reject this manuscript?</h4>
              <p className="text-gray-700">{selectedManuscript.title}</p>
            </div>
            <div className="mt-6 flex justify-end space-x-4">
              <button
